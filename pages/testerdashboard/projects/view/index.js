...
export default function TesterViewProject() {
  ...
  const getParticipantNames = (participantIds) => {
    return participantIds.map(id => {
      const member = team.find(t => t.id === String(id));
  ...
  return (
    <div>
      ...
      <div style={{ padding: '20px' }}>
        <button 
          onClick={() => router.push('/testerdashboard/projects')}
        ...
        <div style={{ background: 'white', padding: '20px', border: '1px solid #ddd', borderRadius: '4px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
            <h3 style={{ margin: 0, fontSize: '20px' }}>My Meetings</h3>
            <button 
          ...
          {meetings.length > 0 ? (
            ...
            <p style={{ color: '#666' }}>No meetings scheduled for you.</p>
          )}
        </div>
      </div>
    </div>
